...
const TAGS: Record<Category, string[]> = {
  ...
  fashion:      ['editorial', 'street', 'minimal', 'couture', 'accessories', 'texture', 'monochrome', 'studio', 'drape'],
};

// Aspect ratios: portrait, landscape, square, wide
const ASPECTS = [
  [400, 600], [600, 400], [400, 400], [800, 450],
  [450, 600], [600, 350], [500, 500], [700, 400],
  [380, 560], [560, 380], [420, 420], [750, 430],
];

const CATS = Object.keys(TAGS) as Category[];
const SEEDS = Array.from({ length: 96 }, (_, i) => (i + 1) * 11);
...
export const GALLERY: GalleryImage[] = SEEDS.map((seed, i) => {
  ...
  return {
    ...
    thumb: `https://picsum.photos/seed/${seed}/${Math.round(w/2)}/${Math.round(h/2)}`,
  };
});
